...
import org.fossify.shiguang.stories.MemoryChrome
...
import org.fossify.shiguang.adapters.MediaAdapter
import org.fossify.shiguang.asynctasks.GetMediaAsynctask
import org.fossify.shiguang.databases.GalleryDatabase
import org.fossify.shiguang.databinding.ActivityMediaBinding
import org.fossify.shiguang.dialogs.ChangeGroupingDialog
import org.fossify.shiguang.dialogs.ChangeSortingDialog
import org.fossify.shiguang.dialogs.ChangeViewTypeDialog
import org.fossify.shiguang.dialogs.FilterMediaDialog
import org.fossify.shiguang.dialogs.GrantAllFilesDialog
import org.fossify.shiguang.extensions.config
import org.fossify.shiguang.extensions.deleteDBPath
import org.fossify.shiguang.extensions.directoryDB
import org.fossify.shiguang.extensions.emptyAndDisableTheRecycleBin
import org.fossify.shiguang.extensions.emptyTheRecycleBin
import org.fossify.shiguang.extensions.favoritesDB
import org.fossify.shiguang.extensions.getCachedMedia
import org.fossify.shiguang.extensions.getHumanizedFilename
import org.fossify.shiguang.extensions.isDownloadsFolder
import org.fossify.shiguang.extensions.launchAbout
import org.fossify.shiguang.extensions.launchCamera
import org.fossify.shiguang.extensions.launchSettings
import org.fossify.shiguang.extensions.launchGesturePlayer
import org.fossify.shiguang.extensions.mediaDB
import org.fossify.shiguang.extensions.movePathsInRecycleBin
import org.fossify.shiguang.extensions.openPath
import org.fossify.shiguang.extensions.openRecycleBin
import org.fossify.shiguang.extensions.restoreRecycleBinPaths
import org.fossify.shiguang.extensions.showRecycleBinEmptyingDialog
import org.fossify.shiguang.extensions.showRestoreConfirmationDialog
import org.fossify.shiguang.extensions.tryDeleteFileDirItem
import org.fossify.shiguang.extensions.updateWidgets
import org.fossify.shiguang.helpers.DIRECTORY
import org.fossify.shiguang.helpers.GET_ANY_INTENT
import org.fossify.shiguang.helpers.GET_IMAGE_INTENT
import org.fossify.shiguang.helpers.GET_VIDEO_INTENT
import org.fossify.shiguang.helpers.GridSpacingItemDecoration
import org.fossify.shiguang.helpers.IS_IN_RECYCLE_BIN
import org.fossify.shiguang.helpers.MAX_COLUMN_COUNT
import org.fossify.shiguang.helpers.MediaFetcher
import org.fossify.shiguang.helpers.PATH
import org.fossify.shiguang.helpers.PICKED_PATHS
import org.fossify.shiguang.helpers.RECYCLE_BIN
import org.fossify.shiguang.helpers.SET_WALLPAPER_INTENT
import org.fossify.shiguang.helpers.SHOW_ALL
import org.fossify.shiguang.helpers.SHOW_FAVORITES
import org.fossify.shiguang.helpers.SHOW_RECYCLE_BIN
import org.fossify.shiguang.helpers.SHOW_TEMP_HIDDEN_DURATION
import org.fossify.shiguang.helpers.SKIP_AUTHENTICATION
import org.fossify.shiguang.helpers.SLIDESHOW_START_ON_ENTER
import org.fossify.shiguang.helpers.VIDEO_PLAYER_APP
import org.fossify.shiguang.helpers.VIDEO_PLAYER_SYSTEM
import org.fossify.shiguang.interfaces.MediaOperationsListener
import org.fossify.shiguang.models.Medium
import org.fossify.shiguang.models.ThumbnailItem
import org.fossify.shiguang.models.ThumbnailSection
import java.io.File
...
class MediaActivity : SimpleActivity(), MediaOperationsListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!(mIsGetImageIntent || mIsGetVideoIntent || mIsGetAnyIntent)) setContentView(MemoryChrome.wrapGallery(this, binding.root.apply { (parent as? android.view.ViewGroup)?.removeView(this) }))
        setupOptionsMenu()
        refreshMenuItems()
        storeStateVariables()
        setupEdgeToEdge(
    ...
    private fun refreshMenuItems() {
        ...
        if (!(mIsGetImageIntent || mIsGetVideoIntent || mIsGetAnyIntent)) MemoryChrome.simplifyMenu(this, binding.mediaMenu.requireToolbar())
    }
    ...
    private fun setupOptionsMenu() {
        ...
        binding.mediaMenu.requireToolbar().setOnMenuItemClickListener { menuItem ->
            when (menuItem.itemId) {
                R.id.memory_appearance -> MemoryChrome.gallerySheet(this, binding.mediaMenu.requireToolbar(), true, false)
                R.id.memory_tools -> MemoryChrome.gallerySheet(this, binding.mediaMenu.requireToolbar(), false, false, mPath == RECYCLE_BIN, config.shouldShowHidden, config.temporarilyShowExcluded)
    ...
    private fun renderMemoryFilters() {
        val bar = binding.memoryMediaFilters
        if (mIsGetImageIntent || mIsGetVideoIntent || mIsGetAnyIntent) { bar.beGone(); return }
        bar.beVisible(); bar.removeAllViews()
        val all = org.fossify.shiguang.helpers.getDefaultFileFilter() or org.fossify.shiguang.helpers.TYPE_PORTRAITS
        val photos = all and org.fossify.shiguang.helpers.TYPE_VIDEOS.inv()
        val options = listOf("全部" to all, "照片" to photos, "视频" to org.fossify.shiguang.helpers.TYPE_VIDEOS)
        options.forEach { (title, filter) ->
            val active = config.filterMedia == filter || (title == "全部" && config.filterMedia == org.fossify.shiguang.helpers.getDefaultFileFilter())
            bar.addView(com.google.android.material.button.MaterialButton(this).apply {
                text = title; textSize = 14f; isAllCaps = false; isSelected = active
                cornerRadius = MemoryChrome.dp(this@MediaActivity, 14); minHeight = MemoryChrome.dp(this@MediaActivity, 48)
                insetTop = 0; insetBottom = 0; elevation = 0f
                val paper = getProperBackgroundColor(); val accent = getProperPrimaryColor(); val ink = getProperTextColor()
                backgroundTintList = android.content.res.ColorStateList.valueOf(if (active) androidx.core.graphics.ColorUtils.blendARGB(paper, accent, .13f) else paper)
                setTextColor(if (active && androidx.core.graphics.ColorUtils.calculateContrast(accent, paper) >= 4.5) accent else ink)
                if (active) typeface = android.graphics.Typeface.DEFAULT_BOLD
                setOnClickListener {
                    if (config.filterMedia != filter) {
                        config.filterMedia = filter; renderMemoryFilters()
                        mLoadedInitialPhotos = false; binding.mediaRefreshLayout.isRefreshing = true
                        binding.mediaGrid.adapter = null; getMedia()
                    }
                }
            }, android.widget.LinearLayout.LayoutParams(0, -2, 1f).apply { marginEnd = MemoryChrome.dp(this@MediaActivity, 6) })
        }
    }
    ...
    private fun setupGridLayoutManager() {
        ...
        if (config.scrollHorizontally) {
            ...
            ).apply { addRule(RelativeLayout.BELOW, R.id.memory_media_filters) }
        } else {
